...
// ver.8 - change int to float variable for temperature 
...
if (limburus == 0)
{
 tft.setTextSize(3);
            ...
            tft.println("Dual thermometer ver.8.0");
            tft.setCursor(80, 80);
...
else
{
 tft.setTextSize(4);
            tft.setCursor(20, 20);
            tft.setTextColor(WHITE);
            tft.println("Termostat dublu 8.0");
            tft.setCursor(30, 80);
            tft.setTextColor(RED);
...
if (tpart7 < (float)tt2-(float)dt/10.){
triunghi(400, 250);
digitalWrite(parter, LOW);
}
if (tpart7 >= (float)tt2)
